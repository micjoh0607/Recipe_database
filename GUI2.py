...
class TkinterShell(tk.Frame):
    def __init__(self, master, **kwargs):
        ...
        self.search_type = ["Ingredients", "Recipes"]
        self.search_flag = 0
        self.master.bind("<Key>", self.key_update)

        self.introduction_text = tk.Label(self.master, text="Welcome Back User!")
        self.introduction_text.grid(row=0, column=0, sticky="nw")

        self.master_button_frame = tk.Frame(self.master, bg="grey")
        ...
        self.master_search_filter_button = tk.Button(self.master, text=self.search_type[self.search_flag],
                                                     command=lambda: self.search_filter())
        self.master_search_filter_button.grid(row=0, column=2)
        self.master_search_box = tk.Entry(self.master)
        ...
        self.main_scroller = ScrollList(self.master, sorted(
            self.data.general_names(self.master_search_box.get(), self.search_type[self.search_flag])), [3, 2], 200,
                                        self.data, self.search_type[self.search_flag], [0,1,2])
        ...
        self.new_recipe_button.grid(row=1, column=0, sticky="nw", padx=5, pady=5)
        self.available_recipe_button = tk.Button(self.master_button_frame, text="Search For Available Recipes", width=25,
                                                 command=lambda: self.available())
    ...
    def ingredients(self, name, window):
        ...
        ingredient_close_button = tk.Button(ingredient_window, text="Close", command=lambda: ingredient_window.destroy())
        ...
        def edit_ingredient():
            for widget in ingredient_introduction_frame.winfo_children():
                widget.destroy()
            ingredient_scroller.update_scroll(sorted(ingredient_name_lst[0].linked_recipes(str(name))), "Ingredient_link", 17, "Recipes", name)
            ...
            ingredient_name_title = tk.Entry(ingredient_introduction_frame,width=30, justify="center")
            ingredient_name_title.grid(row=1, column=0, sticky="nw")
            ingredient_name_title.insert(0, ingredient_name_lst[0].output_name())

            ingredient_quantity_title = tk.Entry(ingredient_introduction_frame, width=20, justify="center")
            ingredient_quantity_title.grid(row=2, column=0, sticky="nw")
            ingredient_quantity_title.insert(0, ingredient_name_lst[0].output_quantity())
            ...
            def save_changes():
                ingredient_name_lst[0].update_name(ingredient_name_title.get().lower())
                ingredient_name_lst[0].update_quantity(ingredient_quantity_title.get())
                ingredient_name_lst[0].update_unit(ingredient_unit_title.get())
                ingredient_name_lst[0].update_increment(update_value.get())

                for widget in ingredient_window.winfo_children():
                    widget.destroy()
        ...
        def commit_change():
            ingredient_name_lst[0].update_quantity(int(new_value.get()))
    ...
    def recipes(self, name, window):
        ...
        def edit_recipe():
            ...
            recipe_name_title = tk.Entry(recipe_introduction_frame, width=30, justify="center")
            recipe_name_title.grid(row=1, column=0, sticky="nw")
            recipe_name_title.insert(0, recipe_name_lst[0].output_name())

            ingredient_scroller.update_scroll(sorted(recipe_name_lst[0].linked_ingredients (str(name))), "Recipe_link", 17, "Recipes", name)
            instruction_scroller.update_scroll(sorted(recipe_name_lst[0].linked_instruction_text(name)), "Instruction_link", 17, "Recipes", name)
            ...
            ingredient_update_button = tk.Button(recipe_window, text="➕", command=lambda: update_recipe(), font=('Arial',9,'bold'))
            ingredient_update_button.grid(row=2, column=0, sticky="ne")
            instruction_update_button = tk.Button(recipe_window, text="➕", command=lambda: update_recipe(), font=('Arial',9,'bold'))
            instruction_update_button.grid(row=4, column=0, sticky="ne")

            def update_recipe():
                self.new_recipe(sorted(recipe_name_lst[0].linked_instruction_text(name)), sorted(recipe_name_lst[0].linked_ingredients(name)), recipe_name_lst[0].output_name(), True)
                recipe_window.destroy()

            def save():
                recipe_name_lst[0].update_name(recipe_name_title.get())

                for widget in recipe_window.winfo_children():
                    widget.destroy()
    ...
    def new_ingredient(self, name, type):
        ...
        def confirm():
            if self.data.check_duplicate(new_ingredient_name_entry.get().lower(), 3) == "unique" and not re.match("^\s*$",
    ...
    def available(self):
        ...
        recipe_list = [self.data.general_names("", "Recipes")[i][0] for i in range(len(self.data.general_names("", "Recipes")))]
        ingredient_list = [self.data.general_names("", "Ingredients")[i][0] for i in range(len(self.data.general_names("" ,"Ingredients")))]
        amalgamation = {recipe_list[i]: [self.data.linked_ingredients(recipe_list[i])[j][0] for j in
                        range(len(self.data.linked_ingredients(recipe_list[i])))] for i in range(len(recipe_list))}
        recipe_reject = []
        scores = []
        ...
        available_recipe_title = tk.Label(available_recipe_window, text="Available Recipes:", font=('Arial',9,'bold','underline'))
        ...
        available_recipe_scroller = ScrollList(available_recipe_window, recipe_list, [2, 0], 100, self.data, "Recipes", [0, 1, 2])
    ...
    def existing_input_error(self, type, name, window):
    ...
    def delete(self, i, format_list, cancel_list, recipe_type, scroller):
        ...
        if recipe_type == "Ingredient_link":
            ...
            scroller.update_scroll(ingredient_list[0].linked_recipes(ingredient_list[0].output_name()), "Ingredient_link", 17, "Recipes", ingredient_list[0].output_name())
        if recipe_type == "Recipe_link":
            ...
            scroller.update_scroll(cancel_list[0].linked_ingredients(cancel_list[0].output_name()), "Recipe_link", 17, "Ingredients", format_list[0].output_name())
        if recipe_type == "Instruction_link":
            cancel_list = [cancel_list]
            cancel_list[0] = recipe(cancel_list[0], "current")
            cancel_list[0].remove_instruction(format_list[0])
            scroller.update_scroll(cancel_list[0].linked_instruction_text(cancel_list[0].output_name()), "Instruction_link", 17,
...
class NewRecipe():
    def __init__(self, window, format_type, data, coordinates, header, width, edit):
        ...
        self.new_recipe_scroller = ScrollList(new_recipe_work_frame, self.new_recipe_list, [2, 1], 100, self.data, self.header, [0,1,2])
    ...
    def user_input(self, event):
        if self.format_type == "Instructions":
            ...
            if not re.match("^\s*$", self.new_recipe_entry.get()) and not re.match("^\s*$", self.new_recipe_quantity.get()):
                if self.data.check_duplicate(self.new_recipe_entry.get(),3) == "unique":
                    main.new_ingredient(self.new_recipe_entry.get(), 1)
                self.new_recipe_list.append((self.new_recipe_entry.get(), self.new_recipe_quantity.get()))
            else:
                main.false_input_error()
        self.new_recipe_scroller.update_scroll(self.new_recipe_list, self.format_type, self.width, self.header, "")
...
class ScrollList():
    def __init__(self, window, button_list, coorindates, height, data, type, order):
        self.window = window
        ...
        for i in range(len(self.button_list)):
            result_button = tk.Button(self.scroll_search_frame,
                                      text=(str(self.button_list[i][self.order[0]]) + " " + str(self.button_list[i][self.order[1]])) + " " + str(self.button_list[i][self.order[2]]), width=20,
                                      command=lambda i=i: main.button_decision(self.type, self.button_list[i][0])).grid(
        ...
        for i in range(len(word_lst)):
            ...
            if format_type == "Instructions":
                ...
                cancel_list[i] = tk.Button(self.scroll_search_frame, text="X",
                                           command=lambda i=i: main.delete(i, [str(word_lst[i][0])], associated_value, format_type, self))
            ...
            if format_type == "Ingredients":
                cancel_list.append(i)
                quantity_list.append(word_lst[i][1])
                cancel_list[i] = tk.Button(self.scroll_search_frame, text="X",
                                           command=lambda i=i: main.delete(i, format_list, cancel_list, "Ingredients", self))
